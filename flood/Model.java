...
class Model {
    ...
    void forNeighbors(Place place, Consumer<Place> action) {
        if (place.row <= height() && place.col <= width()) {

            if (place.row == 0 && place.col == 0) {
                action.accept(pl((place.row) + 1, place.col));
                action.accept(pl(place.row, (place.col) + 1));
            } else if (place.row == 0 && place.col == width() - 1) {
                action.accept(pl(place.row, (place.col) - 1));
                action.accept(pl((place.row) + 1, place.col));
            } else if (place.row == height() - 1 && place.col == 0) {
                action.accept(pl((place.row) - 1, place.col));
                action.accept(pl(place.row, (place.col) + 1));
            } else if (place.row == height() - 1 && place.col == width() - 1) {
                action.accept(pl((place.row) - 1, place.col));
                action.accept(pl(place.row, (place.col) - 1));
            } else if (place.row == 0) {
                action.accept(pl((place.row) + 1, place.col));
            ...
            } else if (place.row == height() - 1) {
                action.accept(pl((place.row) - 1, place.col));
                action.accept(pl(place.row, (place.col) - 1));
                action.accept(pl(place.row, (place.col) + 1));
            } else if (place.col == width() - 1) {
                action.accept(pl((place.row) - 1, place.col));
                action.accept(pl((place.row) + 1, place.col));
                action.accept(pl(place.row, (place.col) - 1));
            } else {
    ...
    final boolean solved() {
        if (activeRegionSize() == width() * height()) {
            return true;
        }
        return false;
    }
    ...
    void setActiveRegionColor(int newColor) {
        if (!_active.contains(Place.pl(0, 0))) {
            _active.add(Place.pl(0, 0));
        }
    ...
    Set<Place> adjacentCells(int color) {
        ...
        HashSet<Place> newSet = new HashSet<>(_active);
        undo();
        newSet.removeAll(_active);
        return newSet;
    }
